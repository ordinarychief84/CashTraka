...
export default async function DebtsPage({ searchParams }: { searchParams: SP }) {
  ...
  const [debts, openAgg, overdueAgg] = await Promise.all([
    ...
    prisma.debt.aggregate({ where: { userId: user.id, status: 'OPEN' }, _sum: { amountOwedKobo: true, amountPaidKobo: true } }),
    prisma.debt.aggregate({ where: { userId: user.id, status: 'OPEN', dueDate: { lt: now, not: null } }, _sum: { amountOwedKobo: true, amountPaidKobo: true }, _count: true }),
  ]);

  const totalOwed = Math.max((openAgg._sum.amountOwedKobo ?? 0) - (openAgg._sum.amountPaidKobo ?? 0), 0);
  const overdueTotal = Math.max((overdueAgg._sum.amountOwedKobo ?? 0) - (overdueAgg._sum.amountPaidKobo ?? 0), 0);
  ...
  return (
    <AppShell businessName={user.businessName} userName={user.name} businessType={user.businessType} accessRole={user.accessRole} principalName={user.principalName}>
      ...
      <div className="mb-4 grid gap-3 md:grid-cols-2">
        <div className="card p-5"><div className="text-xs font-medium text-slate-500">Money owed to you</div><div className="num mt-1 text-3xl text-owed-600">{formatKobo(totalOwed)}</div></div>
        {overdueCount > 0 && (<Link href="/debts?filter=overdue" className="card flex items-center gap-3 p-5 hover:border-owed-500"><span className="flex h-10 w-10 items-center justify-center rounded-full bg-owed-500 text-white"><AlertTriangle size={20} /></span><div className="min-w-0 flex-1"><div className="text-xs font-medium text-slate-500">Overdue</div><div className="text-sm"><span className="font-bold text-ink">{overdueCount} {overdueCount === 1 ? 'debt' : 'debts'}</span> · <span className="num text-owed-600">{formatKobo(overdueTotal)}</span></div></div></Link>)}
      </div>
      ...
      {debts.length === 0 ? (<EmptyState icon={Clock3} title={q || filter !== 'all' ? 'No debts match your search' : 'No debts yet'} description={q || filter !== 'all' ? 'Try a different name, phone or filter.' : 'Log who owes you, and send a reminder with one tap.'} actionHref="/debts/new" actionLabel="Add Debt" />) : (<ul className="space-y-2">{debts.map((d) => { const remainingKobo = Math.max(d.amountOwedKobo - d.amountPaidKobo, 0); const isOverdue = d.status === 'OPEN' && d.dueDate && new Date(d.dueDate) < now; return (<li key={d.id} className="card p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="truncate font-semibold text-ink">{d.customerNameSnapshot}</div><div className="mt-0.5 text-xs text-slate-500">{displayPhone(d.phoneSnapshot)}{d.dueDate ? (<>{' · '}<span className={isOverdue ? 'font-semibold text-owed-700' : ''}>{isOverdue ? 'Overdue ' : 'Due '}{formatDate(d.dueDate)}</span></>) : null}</div>{d.amountPaidKobo > 0 && d.status === 'OPEN' && (<div className="mt-1 text-xs text-slate-600"><span className="num text-success-700">{formatKobo(d.amountPaidKobo)}</span> paid{' · '}<span className="num text-owed-600">{formatKobo(remainingKobo)}</span> remaining</div>)}</div><div className="text-right"><div className={d.status === 'OPEN' ? 'num text-owed-600' : 'num text-slate-500 line-through'}>{formatKobo(d.amountOwedKobo)}</div><span className={d.status === 'OPEN' ? 'badge-open' : 'badge-paid'}>{d.status === 'OPEN' ? 'Open' : 'Paid'}</span></div></div><div className="mt-3"><DebtActions id={d.id} name={d.customerNameSnapshot} phone={d.phoneSnapshot} amountOwed={d.amountOwed} amountPaid={d.amountPaid} status={d.status as 'OPEN' | 'PAID'} /></div></li>); })}</ul>)}
    </AppShell>
  );
}
