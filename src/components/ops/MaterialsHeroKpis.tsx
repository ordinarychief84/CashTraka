...
/**
 * 6 KPI tiles across the top of /materials. Counts every RawMaterial
 * row by stock status, plus rolling stock value + units used this week.
 */
export async function MaterialsHeroKpis({ userId }: { userId: string }) {
  const now = new Date();
  ...
  const [
    ...
    prisma.stockMovement.aggregate({
      where: {
        userId,
        itemType: 'MATERIAL',
        reason: { in: ['PRODUCTION_CONSUME', 'USED', 'WRITE_OFF'] },
  ...
  const stockValueKobo = materialsForValue.reduce(
    (sum, m) => sum + m.stock * m.unitCostKobo,
    0,
  );
  // delta on consume/used is negative — convert to a positive "used" number.
  const unitsUsed = Math.abs(usedThisWeek._sum.delta ?? 0);
  ...
  const tiles: {
    ...
    {
      label: 'Materials Used (7d)',
      value: unitsUsed.toLocaleString('en-NG'),
      delta: null,
      Icon: TrendingUp,
      iconTone: 'brand',
    },
