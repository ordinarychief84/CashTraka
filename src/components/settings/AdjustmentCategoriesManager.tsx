...
export interface AdjustmentCategoryRow {
  id: string;
  name: string;
  type: string | null;
}
...
interface Props {
  initialRows: AdjustmentCategoryRow[];
}

const TYPE_OPTIONS = ['Loss', 'Gain', 'Correction', 'Damage', 'Expiry', 'Write-off'];
...
export function AdjustmentCategoriesManager({ initialRows }: Props) {
  const router = useRouter();
  const [rows, setRows] = useState<AdjustmentCategoryRow[]>(initialRows);
  ...
  async function submit(andNew: boolean) {
    ...
    setError(null);
    try {
      const res = await fetch('/api/settings/adjustment-categories', {
        method: 'POST',
      ...
      setRows((prev) =>
        [...prev, json.data.adjustmentCategory].sort((a, b) => a.name.localeCompare(b.name)),
      );
      if (andNew) {
        reset();
      } else {
  ...
  async function handleDelete(id: string) {
    if (!confirm('Delete this adjustment category? Existing adjustments keep the snapshot name.')) return;
    const res = await fetch(`/api/settings/adjustment-categories/${id}`, { method: 'DELETE' });
    if (res.ok) {
      ...
      {showForm && (
        <div className="mb-4 rounded-xl border border-slate-200 bg-white p-5">
          <div className="mb-4 flex items-center justify-between">
            <p className="text-[15px] font-semibold text-slate-800">Create adjustment category</p>
            <button
              type="button"
              onClick={() => {
                reset();
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
        {rows.length === 0 ? (
          <div className="px-6 py-12 text-center text-[13px] text-slate-500">
            No adjustment categories yet. Add reason codes you'll select when
            recording stock adjustments (damage, expiry, count correction).
          </div>
        ) : (
