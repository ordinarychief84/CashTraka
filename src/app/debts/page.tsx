...
export default async function DebtsPage({ searchParams }: { searchParams: SP }) {
  ...
  const where = {
    ...
    ...(filter === 'overdue'
      ? { status: 'OPEN', dueDate: { lt: now, not: null } }
      : {}),
    ...(q
      ? {
          OR: [
  ...
  const totalOwed = Math.max(
    (openAgg._sum.amountOwed ?? 0) - (openAgg._sum.amountPaid ?? 0),
    0,
  );
    ...
    <AppShell businessName={user.businessName} userName={user.name} businessType={user.businessType} accessRole={user.accessRole} principalName={user.principalName}>
      ...
      <div className="mb-4 grid gap-3 md:grid-cols-2">
        <div className="card p-5">
          ...
          <div className="num mt-1 text-3xl text-owed-600">{formatNaira(totalOwed)}</div>
        </div>
        {overdueCount > 0 && (
          <Link
            href="/debts?filter=overdue"
            className="card flex items-center gap-3 p-5 hover:border-owed-500"
          >
            <span className="flex h-10 w-10 items-center justify-center rounded-full bg-owed-500 text-white">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-sm">
                <span className="font-bold text-ink">
                  {overdueCount} {overdueCount === 1 ? 'debt' : 'debts'}
                </span>{' '}
                · <span className="num text-owed-600">{formatNaira(overdueTotal)}</span>
              </div>
            </div>
          </Link>
        )}
      ...
      <form className="mb-4" action="/debts" method="get">
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        <FilterLink current={filter} value="all" label="All" q={q} />
        <FilterLink current={filter} value="open" label="Open" q={q} />
        <FilterLink current={filter} value="overdue" label="Overdue" q={q} tone="owed" />
        <FilterLink current={filter} value="paid" label="Paid" q={q} />
      </div>

      {debts.length === 0 ? (
        <EmptyState
          icon={Clock3}
          title={q || filter !== 'all' ? 'No debts match your search' : 'No debts yet'}
          description={
            q || filter !== 'all'
              ? 'Try a different name, phone or filter.'
              : 'Log who owes you, and send a reminder with one tap.'
          }
        ...
        <ul className="space-y-2">
          {debts.map((d) => {
            ...
            const isOverdue =
              d.status === 'OPEN' && d.dueDate && new Date(d.dueDate) < now;
            return (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="mt-0.5 text-xs text-slate-500">
                      {displayPhone(d.phoneSnapshot)}
                      {d.dueDate ? (
                        <>
                          {' · '}
                          <span className={isOverdue ? 'font-semibold text-owed-700' : ''}>
                    ...
                    {d.amountPaid > 0 && d.status === 'OPEN' && (
                      <div className="mt-1 text-xs text-slate-600">
                        <span className="num text-success-700">{formatNaira(d.amountPaid)}</span> paid
                        {' · '}
                        <span className="num text-owed-600">{formatNaira(remaining)}</span> remaining
                      </div>
                    )}
                  </div>
                  <div className="text-right">
                    <div
                      className={
                        d.status === 'OPEN'
                          ? 'num text-owed-600'
                          : 'num text-slate-500 line-through'
                      }
                    >
