...
function ScoreRing({ score }: { score: number }) {
  ...
  const color = score >= 70 ? 'text-success-500' : score >= 40 ? 'text-owed-500' : 'text-rose-500';
  const bg = score >= 70 ? 'bg-success-50' : score >= 40 ? 'bg-owed-50' : 'bg-rose-50';
...
export function CollectionScoreWidget({ isPaid }: { isPaid: boolean }) {
  ...
  const trendColor = data.trend === 'up' ? 'text-success-600' : data.trend === 'down' ? 'text-rose-600' : 'text-slate-500';
